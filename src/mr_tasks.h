...
struct BaseMapperInternal {
    ...
    void initialize( std::string output_dir, int shard_id, int num_outputs );
...
inline void BaseMapperInternal::initialize(std::string output_dir, int shard_id, int num_outputs) {
    ...
    for (int i = 0; i < num_outputs; i++) {
        std::string file_name = output_dir + "/temp_shard_" + std::to_string(shard_id) + "_part_" + std::to_string(i);
